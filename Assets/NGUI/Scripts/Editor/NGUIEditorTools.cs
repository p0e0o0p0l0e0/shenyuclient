...
public static class NGUIEditorTools
{
    ...
    //static public void DrawAdvancedTPSpriteField(TexturePackerAtlas tpAtlas, string spriteName, TPSpriteSelector.Callback callback, bool editable,
    //    params GUILayoutOption[] options)
    //{
    //    if (tpAtlas == null) return;

    //    // Give the user a warning if there are no sprites in the atlas
    //    if (tpAtlas.GetListOfSprite().Count == 0)
    //    {
    //        EditorGUILayout.HelpBox("图集中没有sprite", MessageType.Warning);
    //        return;
    //    }
    ...
    //        if (NGUIEditorTools.DrawPrefixButton("Sprite"))
    //        {
    //            NGUISettings.texturePackerAtlas = tpAtlas;
    //            NGUISettings.selectedSprite = spriteName;
    //            TPSpriteSelector.Show(callback);
    //        }
    ...
    //                if (GUILayout.Button("Rename", GUILayout.Width(60f)))
    //                {
    //                    TPSpriteData sprite = tpAtlas.GetSpriteData(spriteName);

    //                    if (sprite != null)
    //                    {
    //                        NGUIEditorTools.RegisterUndo("Edit Sprite Name", tpAtlas);
    //                        sprite.Name = newName;

    //                        List<UIPolygonSprite> sprites = FindAll<UIPolygonSprite>();

    //                        for (int i = 0; i < sprites.Count; ++i)
    //                        {
    //                            UIPolygonSprite sp = sprites[i];

    //                            if (sp.Atlas == tpAtlas && sp.SpriteName == spriteName)
    //                            {
    //                                NGUIEditorTools.RegisterUndo("Edit Sprite Name", sp);
    //                                sp.SpriteName = newName;
    //                            }
