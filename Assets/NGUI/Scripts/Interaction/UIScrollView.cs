...
public class UIScrollView : MonoBehaviour
{
	...
	public OnDragNotification onDragingCallback;
	...
	public virtual bool IsResetPos
	{
		get
		{
			...
			//
			if (contentPivot == UIWidget.Pivot.Left || contentPivot == UIWidget.Pivot.TopLeft || contentPivot == UIWidget.Pivot.BottomLeft)
			{
				if (b.min.x < clip.x - hx - Deviation) return false;
			}
			else if (contentPivot == UIWidget.Pivot.Right || contentPivot == UIWidget.Pivot.TopRight || contentPivot == UIWidget.Pivot.BottomRight)
			{
				if (b.max.x > clip.x + hx + Deviation) return false;
			}
			//
			if (contentPivot == UIWidget.Pivot.Top || contentPivot == UIWidget.Pivot.TopLeft || contentPivot == UIWidget.Pivot.TopRight)
			{
				if (b.max.y > clip.y + hy + Deviation) return false;
			}
			else if (contentPivot == UIWidget.Pivot.Bottom || contentPivot == UIWidget.Pivot.BottomLeft || contentPivot == UIWidget.Pivot.BottomRight)
			{
				if (b.min.y < clip.y - hy - Deviation) return false;
			}
			//
			if (contentPivot == UIWidget.Pivot.Center)
			{
				if (b.center.x != clip.x || b.center.y != clip.y) return false;
			}
			//
			return true;
		}
	...
	protected virtual bool shouldMove
	{
		get
		{
			...
			if (canMoveHorizontally && size.x > 2 * hx)
			{
				if (b.min.x < clip.x - hx) return true;
				if (b.max.x > clip.x + hx) return true;
			}

			if (canMoveVertically && size.y > 2 * hy)
			{
	...
	public virtual void SetDragAmount(float x, float y, bool updateScrollbars)
	{
		...
		if (mPanel.softBorderPadding && mPanel.clipping == UIDrawCall.Clipping.SoftClip)
		{
	...
	protected virtual void LateUpdate()
	{
		...
		if (!mPressed)
		{
			if (mMomentum.magnitude > 0.0001f || mScroll != 0)
			{
				...
				//mScroll = NGUIMath.Lerp(mScroll, 0f, delta);
	...
	public void MoveAbsolute(Vector3 absolute)
	{
		...
		MoveRelative((a - b) * UIRoot.GetPixelSizeAdjustment(gameObject));
	}
	...
	/// </summary>
	public virtual void Scroll(float delta)
	{
	...
	public virtual void Drag()
	{
		...
		if (enabled && NGUITools.GetActive(gameObject) && mShouldMove)
		{
			...
			//
			Vector3 rayPos = smoothDragStart ? (UICamera.currentTouch.pos - mDragStartOffset) : UICamera.currentTouch.pos;
			//rayPos *= uiRoot.pixelSizeAdjustment;
			Ray ray = UICamera.currentCamera.ScreenPointToRay(rayPos);

			float dist = 0f;
			_dragOffest = Vector3.zero;
			if (mPlane.Raycast(ray, out dist))
			{
				Vector3 currentPos = ray.GetPoint(dist);
				_dragOffest = currentPos - mLastPos;
				mLastPos = currentPos;

				if (_dragOffest.x != 0f || _dragOffest.y != 0f || _dragOffest.z != 0f)
				{
					_dragOffest = mTrans.InverseTransformDirection(_dragOffest);
					...
					{
						_dragOffest.Scale((Vector3)customMovement);
					}
					_dragOffest = mTrans.TransformDirection(_dragOffest);
				}

				// Adjust the momentum
				if (dragEffect == DragEffect.None) mMomentum = Vector3.zero;
				else mMomentum = Vector3.Lerp(mMomentum, mMomentum + _dragOffest * (0.01f * momentumAmount), 0.67f);

				// Move the scroll view
				if (!iOSDragEmulation || (dragEffect != DragEffect.MomentumAndSpring))
				{
					MoveAbsolute(_dragOffest);
				}
				else
				{
	...
	protected Vector3 _dragOffest = Vector3.zero;
	...
	public void ResetScrollView(bool forceReset)
	{
		if (forceReset)
		{
			ResetPosition();
