...
///  2017.10.11修改 
/// 1.添加暴露给UI的接口
/// 2.取消sendmessage  保存脚本调用refresh刷新
/// 3.取消LoopScrollDataSource通知刷新的逻辑
/// </summary>
namespace UnityEngine.UI
{
    ...
    public abstract class LoopScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        protected abstract float TotalSize();
        ...
        //对原有逻辑进行扩展  尽量独立出来 不改原有接口
        #region UI逻辑用到的接口

        public bool AutoControlDragable;
        private bool canDrag;
        public Action<string, int> OnRefresh;

        private bool inited;
        private string rootStr;
        private bool filled;

        private Dictionary<int, int> indexDic = new Dictionary<int, int>(); //数据index 索引id 
        private Dictionary<int, int> logicDic = new Dictionary<int, int>(); //索引id  数据index
        private Dictionary<Transform, int> contentDic = new Dictionary<Transform, int>(); //transform 对应索引id

        private List<string> trsList = new List<string>();
        private Stack<Transform> inactiveStack = new Stack<Transform>();
        protected int maxShowNum;

        public void Init(Action<string, int> callback, int totalCount, string root = "",bool canDrag=true)
        {
            ...
            {
                var trs = content.GetChild(i);
                trsList.Add(trs.name);
                if (singleSize == 0)
                    singleSize = GetSize(trs.GetComponent<RectTransform>());
                trs.gameObject.SetActive(false);
                inactiveStack.Push(trs);
                contentDic.Add(trs, i);
                logicDic.Add(i, i);
            }

            inited = true;
            rootStr = root;
            maxShowNum = Mathf.CeilToInt(TotalSize() / singleSize) * GridCount();
        }
        ...
        /// 刷新指定格子
        /// </summary>
        public void RefreshCellByIndex(GameObject go, int index)
        {
//            XLColorDebug.LogUI("刷新  "+ rootStr.ToNodePath(go.name) + "---index="+index);
            if (OnRefresh != null)
                OnRefresh(rootStr.ToNodePath(go.name), index);
//                OnRefresh(GetNodeNameByIndex(index), index);
            else
                Debug.LogError("滑动列表没注册刷新事件");
        }
        ...
        public string GetNodeNameByIndex(int index)
        {
            ...
            {
                int realIndex = indexDic[index];
                if (realIndex < trsList.Count)
                    return rootStr.ToNodePath(trsList[realIndex]);
            }
...
//            }
            ...
            Debug.Log(string.Format("需要刷新的列表数据当前没显示  索引{0}", index));
            return null;
        }

        public int GetIndexByTransform(Transform trs)
        {
            if (logicDic.ContainsKey(contentDic[trs]))
                return logicDic[contentDic[trs]];
            Debug.LogError(string.Format("滑动列表 点到了未知的东西{0} 不在缓存的数据中", trs));
            return 0;
        }

        public Transform GetTransformByIndex(int vIndex)
        {
            foreach (var pair in contentDic)
            {
                if (pair.Value == vIndex)
                    return pair.Key;
            }
        ...
        private Transform GetInactiveTrs()
        {
            if (inactiveStack.Count > 0)
                return inactiveStack.Pop();
            Debug.LogError("滑动列表子对象少了 editor添加一个");
            return null;
        }
        ...
        private void ReturnObjectAndSendMessage(Transform go)
        {
            ...
            //            ScrollPoolManager.Instance.ReturnObjectToPool(go.gameObject);
            int index = contentDic[go];
//            XLColorDebug.LogUI("delete at "+ logicDic[index]);
            indexDic.Remove(logicDic[index]);
        ...
        private void RefreshCells()
        {
            if (Application.isPlaying && this.isActiveAndEnabled)
            {
                ...
                for (int i = 0; i < content.childCount; i++)
                {
                    ...
                    {
                        RefreshCellByIndex(content.transform.GetChild(i).gameObject,itemTypeEnd);
...
//                        if ((totalCount >= 0 && itemTypeEnd >= totalCount - 1) || content.childCount == 0)
            if (content.childCount <= maxShowNum)
            {
                return 0;
            }
...
//                if (itemTypeEnd % contentConstraintCount == 0 || content.childCount == 0)
        ...
        private RectTransform InstantiateNextItem(int itemIdx)
        {
            ...
            RectTransform nextItem = GetInactiveTrs().GetComponent<RectTransform>();
            if(inactiveStack.Count == 0)
                XLColorDebug.LogUI("缓存背包格子用尽。。。");
//            if(indexDic.ContainsKey(itemIdx))
//                XLColorDebug.LogUI("error   index   out range   "+itemIdx);
        //    else
//                XLColorDebug.LogUI("add at " + itemIdx);
            indexDic.SmartAdd(itemIdx, contentDic[nextItem]);
            logicDic[contentDic[nextItem]] = itemIdx;
            nextItem.transform.SetParent(content, false);
            nextItem.gameObject.SetActive(true);
            RefreshCellByIndex(nextItem.gameObject,itemIdx);
            ...
            if (!m_Dragging && (offset != Vector2.zero || m_Velocity != Vector2.zero))
            {
                ...
                for (int axis = 0; axis < 2; axis++)
                {
                    ...
                    else if (m_Inertia)
                    {
                        ...
                        if (Mathf.Abs(m_Velocity[axis]) < 50)
                            m_Velocity[axis] = 0;
