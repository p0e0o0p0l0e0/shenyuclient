...
[CustomEditor(typeof(ExUIButton), true)]
[CanEditMultipleObjects]

public class UIExButtonInspector : SelectableEditor
{
        ...
        ++EditorGUI.indentLevel;
        {
            ...
            if (EditorGUILayout.BeginFadeGroup(m_ShowSpriteTrasition.faded))
            {
                ...
                if (trans == Selectable.Transition.SpriteSwap)
                {
                    ...
                    {
                        ExUISprite sprite = Selection.gameObjects[0].GetComponent<ExUISprite>();
                        if (sprite != null)
                        {
                            SerializedProperty sp = serializedObject.FindProperty("mNormalSprite");
                            EditorGUILayout.BeginHorizontal();
                            EditorGUILayout.LabelField("NormalSprite", GUILayout.Width(120f));
                            NGUIEditorTools.DrawAdvancedSpriteField(sprite.Atlas, sp.stringValue, this.SelectNormalSprite, false);
                            EditorGUILayout.EndHorizontal();

                            EditorGUILayout.BeginHorizontal();
                            EditorGUILayout.LabelField("PressSprite", GUILayout.Width(120f));
                            sp = serializedObject.FindProperty("mPressSprite");
                            NGUIEditorTools.DrawAdvancedSpriteField(sprite.Atlas, sp.stringValue, this.SelectPressSprite, false);
                            EditorGUILayout.EndHorizontal();

                            EditorGUILayout.BeginHorizontal();
                            EditorGUILayout.LabelField("DisableSprite", GUILayout.Width(120f));
                            sp = serializedObject.FindProperty("mDisableSprite");
                            NGUIEditorTools.DrawAdvancedSpriteField(sprite.Atlas, sp.stringValue, this.SelectDisabelSprite, false);
                            EditorGUILayout.EndHorizontal();
                        }
                    }
    ...
    void SelectPressSprite(string spriteName)
    {
        serializedObject.Update();
        SerializedProperty sp = serializedObject.FindProperty("mPressSprite");
        sp.stringValue = spriteName;
        serializedObject.ApplyModifiedProperties();
        NGUITools.SetDirty(serializedObject.targetObject);
        NGUISettings.selectedSprite = spriteName;
    }
    void SelectDisabelSprite(string spriteName)
    {
        serializedObject.Update();
        SerializedProperty sp = serializedObject.FindProperty("mDisableSprite");
        sp.stringValue = spriteName;
        serializedObject.ApplyModifiedProperties();
        NGUITools.SetDirty(serializedObject.targetObject);
        NGUISettings.selectedSprite = spriteName;
    }
    private void DrawTransitionSprite()
    { }
