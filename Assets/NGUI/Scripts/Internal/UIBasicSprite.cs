...
public abstract class UIBasicSprite : UIWidget
{
	...
	[HideInInspector]
	[SerializeField]
	protected float mFillAmount = 1.0f;
	[HideInInspector]
	[SerializeField]
	protected bool mInvert = false;
	[HideInInspector]
	[SerializeField]
	...
	void SlicedFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		verts.ReserveCount(3 * 3 * 4);
		uvs.ReserveCount(3 * 3 * 4);
		cols.ReserveCount(3 * 3 * 4);
	...
	void TiledFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		/// <ReserveCount>
		int countX = (int)System.Math.Ceiling((v.z - v.x) / size.x);
		int countY = (int)System.Math.Ceiling((v.w - v.y) / size.y);
		int count = countX * countY * 4;
		verts.ReserveCount(count);
		uvs.ReserveCount(count);
		cols.ReserveCount(count);
		/// </ReserveCount>
	...
	void FilledFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		if (mFillAmount < 1f)
		{
			...
			{
				verts.ReserveCount(2 * 4);
				uvs.ReserveCount(2 * 4);
				cols.ReserveCount(2 * 4);
			...
			{
				verts.ReserveCount(4 * 4);
				uvs.ReserveCount(4 * 4);
				cols.ReserveCount(4 * 4);
	...
	void AdvancedFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		/// <ReserveCount>
		int countX = (int)System.Math.Ceiling((v.z - v.x) / tileSize.x);
		int countY = (int)System.Math.Ceiling((v.w - v.y) / tileSize.y);
		int count = countX * countY * 4 + 2;
		verts.ReserveCount(count);
		uvs.ReserveCount(count);
		cols.ReserveCount(count);
		/// </ReserveCount>
