...
public class UIToggle : UIWidgetContainer
{
	...
	public delegate bool ClickDele(UIToggle toggle, bool isSelect);
	public ClickDele OnToggleClicked;
	public delegate void SelectDele(UIToggle toggle, bool select);
	public SelectDele OnToggleSelected;
	public SelectDele OnPlayChangeSound;
	...
	public bool value
	{
		...
		set
		{
			...
			else if (group == 0 || value || optionCanBeNone || !mStarted) Set(value, true);
		}
	}

	/// <summary>
	/// Whether the toggle is checked. ( Without executing Callback)
	/// </summary>
	public bool isChecked
	{
		...
		{
			if (!mStarted) startsActive = value;
			else if (group == 0 || value || optionCanBeNone || !mStarted) Set(value, false);
		}
	}
	...
	void OnClick() 
	{
		...
		{
			if (OnToggleClicked == null || OnToggleClicked(this, value))
			{
				value = !value;
			}
		}
	...
	/// </summary>
	public void Set(bool state, bool execute)
	{
		...
		else if (mIsActive != state)
		{
			...
			if (group != 0 && state)
			{
				...
				{
					UIToggle cb = list[i];
					if (cb != this && cb.group == group) cb.Set(false, false);
			...
			if (activeSprite != null)
			{
				...
				else
				{
					...
					{
						OnToggleSelected(this, mIsActive);
					}
					if (OnPlayChangeSound != null)
					{
						OnPlayChangeSound(this, mIsActive);
					}
				}
			}
