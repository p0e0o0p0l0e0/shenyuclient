...
public class UIPanel : UIRect
{
	...
	static public int RenderQSpanScale = 5;
...
#endif
		{
			...
			for (int i = 0, imax = list.Count; i < imax; ++i)
			{
				...
				if (p.renderQueue == RenderQueue.Automatic)
				{
					...
					p.UpdateDrawCalls();
					RenderQSpanScale = Mathf.Max(1, RenderQSpanScale);
					rq += p.drawCalls.Count * RenderQSpanScale;
				}
				else if (p.renderQueue == RenderQueue.StartAt)
				{
					...
					{
						RenderQSpanScale = Mathf.Max(1, RenderQSpanScale);
						rq = Mathf.Max(rq, p.startingRenderQueue + p.drawCalls.Count * RenderQSpanScale);
					}
				}
				else // Explicit
				{
