...
public class UIWidgetInspector : UIRectEditor
{
	...
	static public void DrawInspectorProperties (SerializedObject so, UIWidget w, bool drawColor)
	{
		...
		if (NGUIEditorTools.DrawHeader("Widget"))
		{
			...
			{
				SerializedProperty autoResizeProperty;
				GUILayout.BeginHorizontal();
				{
					autoResizeProperty = NGUIEditorTools.DrawProperty("Collider", so, "autoResizeBoxCollider", GUILayout.Width(100f));
					GUILayout.Label("auto-adjust to match");
				}
				GUILayout.EndHorizontal();
				if (autoResizeProperty.boolValue)
				{
					NGUIEditorTools.DrawProperty("SizeAppend", so, "ColliderSizeAppend");
				}
			}
