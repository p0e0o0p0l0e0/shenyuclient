...
Shader "Fate Shading/RadialBlur" {
Properties {
	...
	CGINCLUDE
		...
		{
			fixed2 dir =_Center.xy-i.texcoord;
		...
		{
			// fixed2 dir = _Center.xy-i.texcoord;
			fixed dist = length(_Center.xy-i.texcoord);
			fixed4  col = tex2D(_MainTex, i.texcoord);
			fixed4  blur = tex2D(_BlurTex, fixed2(i.texcoord.x,  i.texcoord.y));
