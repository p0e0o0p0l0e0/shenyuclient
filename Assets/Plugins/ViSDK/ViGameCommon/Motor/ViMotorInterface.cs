...
public class ViMotorInterface
{
	...
	public bool Update(float deltaTime, out ViVector3 moved)
	{
		...
		deltaTime = ViMathDefine.Min(_duration, deltaTime);
		while (deltaTime > _iterSpan)
		{
			deltaTime -= _iterSpan;
			ViVector3 iterMoved;
			_Update(_iterSpan, out iterMoved);
			moved += iterMoved;
		}
		if (deltaTime > 0)
		{
			ViVector3 iterMoved;
			_Update(deltaTime, out iterMoved);
			moved += iterMoved;
		}
		return moved.Length2 > 0.0001f;
	}
	void _Update(float deltaTime, out ViVector3 moved)
	{
		ViDebuger.AssertError(_target);
