...
public static class ViVector3Serialize
{
	...
	public static void Append(ViStringDictionaryStream OS, string head, ViVector3 value)
	{
		ViSerializer<float>.Append(OS, head + "x", value.x);
		ViSerializer<float>.Append(OS, head + "y", value.y);
		ViSerializer<float>.Append(OS, head + "z", value.z);
	}
	public static void Read(ViStringDictionaryStream IS, string head, out ViVector3 value)
	{
		value = new ViVector3();
		ViSerializer<float>.Read(IS, head + "x", out value.x);
		ViSerializer<float>.Read(IS, head + "y", out value.y);
		ViSerializer<float>.Read(IS, head + "z", out value.z);
	}
	...
	public static void PrintTo(List<ViVector3> list, ref string strValue)
	{
		strValue += "(";
		for (int iter = 0; iter < list.Count; ++iter)
		{
			ViVector3 value = list[iter];
			PrintTo(value, ref strValue);
			strValue += ",";
		}
		strValue += ")";
	}
