...
public class UIPanelInspector : UIRectEditor
{
	...
	protected override bool ShouldDrawProperties ()
	{
		...
		if (NGUIEditorTools.DrawHeader("Advanced Options"))
		{
			...
			//else
			//{
			//    GUILayout.Label("Render Q Span Scale", GUILayout.Width(80f));
			//    int rqSpanScale = EditorGUILayout.IntField(mPanel.RenderQSpanScale, GUILayout.Width(40f));
			//    if (mPanel.RenderQSpanScale != rqSpanScale)
			//    {
			//        mPanel.RenderQSpanScale = rqSpanScale;
			//        mPanel.RebuildAllDrawCalls();
			//        EditorUtility.SetDirty(mPanel);
			//        if (UIDrawCallViewer.instance != null)
			//            UIDrawCallViewer.instance.Repaint();
