...
public class UICamera : MonoBehaviour
{
	...
	public static float InertiaDeltaScale = 2.5f;
	public static Vector2 CurrentDelta()
	{
		return currentTouch.delta * InertiaDeltaScale;
	}
	public static float CurrentInertiaVelocity(bool horizontal)
	{
		Vector2 delta = UICamera.CurrentDelta();
		if (horizontal)
		{
			return delta.x / currentTouch.deltaTime;
		}
		else
		{
			return delta.y / currentTouch.deltaTime;
		}
	}
	...
	public ThresholdType ThresholdStyle = ThresholdType.PIXEL;
	...
	public static GameObject Raycast(UnityEngine.Touch touch)
	{
		if (!Raycast(touch.position)) mRayHitObject = fallThrough;
		if (mRayHitObject == null) mRayHitObject = mGenericHandler;
		return mRayHitObject;
	}
	...
	public void ProcessTouches ()
	{
		...
		for (int iter = 0; iter < count; ++iter)
		{
			...
			{
				UnityEngine.Touch touch = Input.GetTouch(iter);
				phase = touch.phase;
			...
			{
				Touch touch = GetInputTouch(iter);
				phase = touch.phase;
				fingerId = touch.fingerId;
				position = touch.position;
				tapCount = touch.tapCount;
			...
			//
			currentTouch.Touch = Input.GetTouch(iter);
	...
	void ProcessPress (bool pressed, float click, float drag)
	{
		...
		else if (currentTouch.pressed != null && (currentTouch.delta.sqrMagnitude != 0f || currentTouch.current != currentTouch.last))
		{
			...
			else if (!currentTouch.dragStarted && drag < mag) // Custom Code : || currentTouch.deltaTime > DragDelay
			{
	...
	void ProcessRelease (bool isMouse, float click)
	{
		...
		if (currentTouch.pressed != null)
		{
			...
			if (currentTouch.dragged == currentTouch.current ||
				(currentScheme != ControlScheme.Controller &&
				currentTouch.clickNotification != ClickNotification.None &&
				currentTouch.totalDelta.sqrMagnitude < click))
			{
	...
	public void ProcessTouch (bool pressed, bool released)
	{
		...
		if (ThresholdStyle == ThresholdType.REAL)
		{
			drag = drag * Screen.dpi / INCH_TO_CM;
			click = click * Screen.dpi / INCH_TO_CM;
		}
		...
		if (currentTouch.pressed != null)
		{
			if (released) ProcessRelease(isMouse, click);
