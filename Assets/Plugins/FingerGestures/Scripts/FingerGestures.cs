...
public class FingerGestures : MonoBehaviour
{
    ...
    public static bool IsTouchUI(Finger pFinger)
    {
        return IsTouchUI(pFinger.StartPosition);
    }

    public static bool IsTouchUI(Vector2 vScreenPos)
    {
        ...
        eventDataCurrentPosition.position = vScreenPos;
    ...
    public static FingerGestures.FingerList GetValidTouchs()
    {
        FingerGestures.FingerList list = new FingerList();
        for (int i = 0; i < Touches.Count; ++i)
        {
            if (Touches[i].IsDown && !FingerGestures.IsTouchUI(Touches[i].StartPosition))
            {
                list.Add(Touches[i]);
            }
        }
