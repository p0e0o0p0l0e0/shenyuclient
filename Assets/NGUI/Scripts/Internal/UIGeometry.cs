...
public class UIGeometry
{
	...
	public void ApplyTransform (Matrix4x4 widgetToPanel, bool generateNormals = true)
	{
		...
		{
			mRtpVerts.Clear();
			mRtpVerts.ReserveCount(verts.size);
	...
	public void WriteToBuffers (BetterList<Vector3> v, BetterList<Vector2> u, BetterList<Color32> c, BetterList<Vector3> n, BetterList<Vector4> t)
	{
		if (mRtpVerts != null && mRtpVerts.size > 0)
		{
			if (n == null)
			{
				v.ReserveCount(mRtpVerts.size);
				u.ReserveCount(mRtpVerts.size);
				c.ReserveCount(mRtpVerts.size);
			...
			else
			{
				...
				u.ReserveCount(mRtpVerts.size);
				c.ReserveCount(mRtpVerts.size);
				n.ReserveCount(mRtpVerts.size);
				t.ReserveCount(mRtpVerts.size);
				for (int i = 0; i < mRtpVerts.size; ++i)
				{
					v.Add(mRtpVerts.buffer[i]);
					u.Add(uvs.buffer[i]);
