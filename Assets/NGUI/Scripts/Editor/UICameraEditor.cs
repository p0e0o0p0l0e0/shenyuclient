		...
		else
		{
			...
			if ((mouse.boolValue || touch.boolValue) && NGUIEditorTools.DrawHeader("Thresholds"))
			{
				NGUIEditorTools.BeginContents();
				{
					EditorGUI.BeginDisabledGroup(!mouse.boolValue);
					GUILayout.BeginHorizontal();
					cam.ThresholdStyle = (UICamera.ThresholdType)EditorGUILayout.EnumPopup(cam.ThresholdStyle);
					string unitType;
					if (cam.ThresholdStyle == UICamera.ThresholdType.PIXEL)
					{
						unitType = "pixels";
					}
					else
					...
					GUILayout.BeginHorizontal();
					EditorGUILayout.PropertyField(serializedObject.FindProperty("touchClickThreshold"), new GUIContent("Touch Click"), GUILayout.Width(120f));
					GUILayout.Label(unitType);
					GUILayout.EndHorizontal();
					EditorGUI.EndDisabledGroup();
				}
