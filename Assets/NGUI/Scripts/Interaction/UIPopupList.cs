...
public class UIPopupList : UIWidgetContainer
{
	...
	static protected GameObject popupListObj;
	...
	[HideInInspector]
	[SerializeField]
	protected UIPanel mParentPanel;
	[HideInInspector]
	[SerializeField]
	...
	[HideInInspector]
	[SerializeField]
	protected UILabel mHighlightedLabel = null;
	[HideInInspector]
	[SerializeField]
	protected List<UILabel> mLabelList = new List<UILabel>();
	[HideInInspector]
	[SerializeField]
	protected float mBgBorder = 0f;

	[System.NonSerialized]
	protected GameObject mSelection;
	[System.NonSerialized]
	protected int mOpenFrame = 0;
	...
	static public bool isOpen { get { return current != null && (popupListObj != null || mFadeOutComplete > Time.unscaledTime); } }
	...
	public int Index
	{
		get { return items.IndexOf(mSelectedItem); }
		set
		{
			if (items.Count > 0)
			{
				int idx = Mathf.Clamp(value, 0, items.Count - 1);
	...
	public bool isValid { get { return bitmapFont != null || trueTypeFont != null; } }

	/// <summary>
	/// Active font size.
	/// </summary>

	public int activeFontSize { get { return (trueTypeFont != null || bitmapFont == null) ? fontSize : bitmapFont.defaultSize; } }

	/// <summary>
	/// Font scale applied to the popup list's text.
	/// </summary>

	public float activeFontScale { get { return (trueTypeFont != null || bitmapFont == null) ? 1f : (float)fontSize / bitmapFont.defaultSize; } }
	...
	public void SetItems(List<string> itemList, int select)
	{
		SetItems(itemList, new List<object>(), 0);
	}
	public void SetItems(List<string> itemList, List<object> itemDataList)
	{
		SetItems(itemList, itemDataList, 0);
	}
	public void SetItems(List<string> itemList, List<object> itemDataList, int select)
	{
		if (itemList != null && itemList.Count > 0)
		{
			...
			//
			if (select < 0 || select >= itemList.Count)
			{
				select = 0;
			}
			value = items[select];
	...
	[System.NonSerialized]
	protected bool mExecuting = false;
	...
	{
		if (popupListObj != null && current == this)
		{
			...
			if (isAnimated)
			{
				UIWidget[] widgets = popupListObj.GetComponentsInChildren<UIWidget>();
	...
	public virtual void Show()
	{
		if (enabled && NGUITools.GetActive(gameObject) && popupListObj == null && atlas != null && isValid && items.Count > 0)
		{
			...
			if (separatePanel)
			{
				...
				popupListObj.AddComponent<UIPanel>().depth = 1000000;
			}
			current = this;

			Transform t = popupListObj.transform;
			t.parent = mParentPanel.cachedTransform;
			Vector3 pos;
			...
			mHighlight = NGUITools.AddSprite(popupListObj, atlas, highlightSprite, mBackground.depth + 1);
			...
			for (int i = 0, imax = items.Count; i < imax; ++i)
			{
				...
				UILabel lbl = NGUITools.AddWidget<UILabel>(popupListObj, mBackground.depth + 2);
				lbl.name = i.ToString();
